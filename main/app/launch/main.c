...
void app_launch_init() {
	...
	ui_item_t * phone = ui_button_new("Phone", 10, 20 + 5, 32, 32, app_launch_button_click);
	list_add(app_launch_main->fg, phone);

	ui_item_t * sms = ui_button_new("SMS", 10, 20 + 5 + ((32 + 10) * 1), 32, 32, app_launch_button_click);
	list_add(app_launch_main->fg, sms);

	ui_item_t * other = ui_button_new("Other", 10, 20 + 5 + ((32 + 10) * 2), 32, 32, app_launch_button_click);
